...
def makeStemsList(f, g_hPoints, g, italicAngle, minStemX, minStemY, maxStemX, maxStemY, roundFactor_Stems):
	...
	for (i1, i2) in pairs:
		...
		if goodCandidate(angleIn_source, angleOut_target):
			goodAngle = angleIn_source
		if goodCandidate(angleOut_source, angleIn_target):
			goodAngle = angleOut_source
		if goodCandidate(angleOut_source, angleOut_target):
			goodAngle = angleOut_source # so if both in and out are good, we keep only one, namely 'out'
