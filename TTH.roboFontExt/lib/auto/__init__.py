...
def makeStemsList(g, contours, italicAngle, xBound, yBound, roundFactor_Stems, tolerance, dedup=True):
	...
	for gidx, (sc, ss) in enumerate(contsegs): # 'sc' = Source Contour, 'sc' = Source Segment
		...
		for (tc, ts) in contsegs[gidx+1:]: # 'tc' = Target Contour, 'tc' = Target Segment
			...
			c_distance = geom.Point( helperFunctions.roundbase(dif.x, roundFactor_Stems), helperFunctions.roundbase(dif.y, roundFactor_Stems) )
			hypoth = dif.length()
			#if debug: print "AA"
	...
	for i, stems in enumerate([stemsListX_temp, stemsListY_temp]):
		...
		for (hypoth, stem) in stems:
			sourceAbsent = not helperFunctions.exists(references, lambda y: helperFunctions.approxEqual(stem[0].shearedPos[i], y, 0.025))
			targetAbsent = not helperFunctions.exists(references, lambda y: helperFunctions.approxEqual(stem[1].shearedPos[i], y, 0.025))
